...
#    cha_gif = tk.PhotoImage(file=".\\cha.gif")
    ...
    os.remove('tmp.gif')
    ttk.Label(software_frame, text="Android Photo Version 1.1").grid(row=0, column=1, sticky = W)
    ttk.Label(software_frame, text="@Author    :   lijishi").grid(row=1, column=1, sticky = W)
    ttk.Label(software_frame, text="@EditTime  :   Feb 20,2020").grid(row=2, column=1, sticky = W)
...
# window layout
main_window.title("Android Photo V1.1")
with open('tmp.ico', 'wb') as tmp:
    tmp.write(base64.b64decode(Icon().img))
